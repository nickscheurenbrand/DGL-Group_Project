...
class BrainGraphDataset(Dataset):
    ...
    def _preprocess(self) -> List[Tuple[Data, Data]]:
        ...
        for idx in range(self.n_samples):
            ...
            lr_matrix = MatrixVectorizer.anti_vectorize(
                lr_vector, self.lr_nodes, include_diagonal=False
            )
            hr_matrix = MatrixVectorizer.anti_vectorize(
                hr_vector, self.hr_nodes, include_diagonal=False
            )
...
if __name__ == "__main__":
    dataset = BrainGraphDataset(
        lr_file="generated_data/lr_train.csv", hr_file="generated_data/hr_train.csv"
    )
    print(dataset[0][0])
    print(dataset[0][1])
